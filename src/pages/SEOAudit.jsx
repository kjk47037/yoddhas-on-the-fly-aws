...
export default function SEOAudit() {
  ...
  const pollAuditProgress = async (auditId) => {
    const pollInterval = setInterval(async () => {
      try {
        const response = await fetch(`${API_URL}/api/seo/audit/${auditId}/status`);
        ...
        if (status.status === 'completed') {
          ...
          const resultsResponse = await fetch(`${API_URL}/api/seo/audit/${auditId}`);
  ...
  const exportAuditReport = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/seo/export/${auditData.auditId}/json`);
